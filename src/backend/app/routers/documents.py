...
from app.services.document_cleanup import (
    cleanup_bank_statement_document,
    cleanup_receipt_document,
    schedule_s3_deletes,
)
...
async def delete_document(
    ...
    if doc.document_type == DocumentType.receipt and doc.receipt_id is not None:
        await cleanup_receipt_document(doc, db)
    elif (
        doc.document_type == DocumentType.bank_statement
        and doc.statement_id is not None
    ):
        receipt_docs = await cleanup_bank_statement_document(doc, db)

    doc.soft_delete()
    await db.commit()

    schedule_s3_deletes(background_tasks, aws_service, [doc, *receipt_docs])
