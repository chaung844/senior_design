...
- POST /reconciliation/jobs/{job_id}/run: trigger run; status is read via GET /jobs/{job_id}/status.
...
async def run_reconciliation(
    ...
    """
    Trigger the reconciliation algorithm for a job.
    Job status is read via GET /jobs/{job_id}/status.
    When job_documents exists, scope can come from the job; until then, pass account_id in body to run.
    """
    ...
    # Load statement lines and receipts for this account (user already has access)
    lines_stmt = (
    ...
    for line in lines:
        ...
        for receipt in receipts:
            if receipt.match_status != MatchStatus.unmatched:
                continue
            if line.charge == receipt.charged_amount and line.transaction_date == receipt.billing_date:
...
async def create_manual_match(
    ...
    from app.utils.access import get_owned_statement_line, get_owned_receipt
...
async def delete_match(
    ...
    remaining_line = await db.execute(
        select(func.count()).select_from(ReconciliationMatch).where(
            ReconciliationMatch.line_id == line_id
        )
    )
    ...
    if receipt:
        remaining_receipt = await db.execute(
            select(func.count()).select_from(ReconciliationMatch).where(
                ReconciliationMatch.receipt_id == receipt_id
            )
        )
