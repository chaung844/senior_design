...
| **Tier 5** — Accounts | `POST /accounts`, `GET /accounts` (optional `provisioned_tenant_only` for developers), `GET /accounts/{id}`, `PATCH /accounts/{id}`, `DELETE /accounts/{id}`, `GET /accounts/{id}/export/vendor-sheet` (vendor sheet CSV/ZIP by month range) | `useAccounts()`, `useAccount()`, `useAccountBook()`, `useAccountBooks()`, `useProvisionedTenantAccounts()`, `useCreateAccount()`, `useUpdateAccount()`, `useDeleteAccount()`; `downloadAccountVendorSheet()` in `lib/api.ts` |
